...
const Workspaces = () => {
  ...
  const [isWorkspaceAvailable, setIsWorkspaceAvailable] = useState();

  // console.log('formvalue workspace', formValue);
  ...
  const CheackWorkspaceAvailibility = () => {
    ...
    API.workspace
      ...
      .then((response) => {
        if (response) setIsWorkspaceAvailable(true);
      })
      .finally(() => {
